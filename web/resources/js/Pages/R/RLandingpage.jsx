...
/* ------- Mini Toggle (mirip TransactionToggle) ------- */
const TypeToggle = ({ value, onChange }) => {
    return (
        <div className="w-full">
            <div className="grid grid-cols-2 gap-3 sm:gap-4">
                {[
                    { key: "Pengeluaran", color: "red" },
                    { key: "Pemasukan", color: "emerald" },
                ].map((opt) => {
                    ...
                    return (
                        <button
                            ...
                            className={`relative h-14 sm:h-16 rounded-xl sm:rounded-2xl border-2 transition-all duration-300 ease-in-out transform active:scale-95 hover:scale-105 ${
                                active
                                    ? `border-${opt.color}-400 bg-${opt.color}-50/80 shadow-lg shadow-${opt.color}-200/50`
                        ...
                        >
                            ...
                            {active && (
                                <div className="absolute inset-0 rounded-xl sm:rounded-2xl bg-black/0 pointer-events-none"></div>
                            )}
                        </button>
                    );
                })}
                ...
                return (
                    <button
                        ...
                        className={`px-3 py-1.5 rounded-xl border-2 text-sm font-medium transition-all hover:scale-[1.03] ${
                            active
                                ? `bg-${p.color}-500 text-white border-${p.color}-600`
                                : `border-${p.color}-200 text-${p.color}-700 bg-white`
                        }`}
                    >
                        {p.key}
                    </button>
...
const CategoryCard = ({ cat, onEdit, onDelete }) => {
    ...
    return (
        <div
            className={`group bg-white/70 backdrop-blur-xl rounded-2xl p-4 border border-white/30 shadow-sm ring-2 ${ring} transition`}
        >
                ...
                <div className="flex items-center gap-3 min-w-0">
                    ...
                    <div className="min-w-0">
                        <div className="flex items-center gap-2">
                            ...
                            <span
                                className={`text-[10px] px-2 py-0.5 rounded-lg border ${badge}`}
                            >
                                {cat.priority}
                            </span>
                        </div>
        ...
        <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center p-4 bg-black/30">
            <div className="w-full max-w-lg bg-white/80 backdrop-blur-xl rounded-2xl border border-white/30 shadow-xl">
                ...
                <div className="p-4 border-t border-white/40 flex gap-2">
                    <button
                        onClick={onClose}
                        className="flex-1 py-2.5 rounded-xl font-medium bg-slate-100 text-slate-700 hover:bg-slate-200"
                    >
                        Batal
                    </button>
                    <button
                        onClick={onSubmit}
                        disabled={disabled}
                        className={`flex-1 py-2.5 rounded-xl text-white font-semibold shadow-lg transition ${
                            disabled
...
const RLandingPage = () => {
    ...
    const loadCategories = async () => {
        setLoading(true);
        try {
            const res = await api.get("/category"); // IMPORTANT: '/category' not '/categories'
    ...
    const valid =
        name.trim() &&
        ["Pemasukan", "Pengeluaran"].includes(type) &&
    ...
    const handleSubmit = async () => {
        ...
        try {
            if (mode === "create") {
                ...
                const res = await api.put(`/category/${currentId}`, {
                    name,
                    icon,
                    type,
                    priority,
                ...
                const payload = res.data?.data || res.data;
                setCategories((prev) =>
                    prev.map((c) => (c.id === currentId ? payload : c))
                );
    ...
    return (
        <div className="w-full min-h-screen bg-gradient-to-br from-emerald-50 via-teal-50 to-cyan-100 text-slate-800 relative overflow-hidden">
            <div className="absolute inset-0 overflow-hidden">
                ...
                <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-32 sm:w-64 h-32 sm:h-64 bg-gradient-to-br from-teal-200/20 to-emerald-300/20 rounded-full blur-3xl animate-pulse delay-500"></div>
            </div>
                        ...
                        <div className="animate-pulse space-y-3">
                            ...
                            <div className="grid sm:grid-cols-2 gap-3">
                                {[...Array(4)].map((_, i) => (
                                    <div
                                        key={i}
                                        className="h-20 bg-slate-200/60 rounded-xl"
                                    ></div>
                                ))}
                            </div>
                        </div>
                    ...
                    <div className="grid sm:grid-cols-2 gap-3 sm:gap-4">
                        ...
                        {filtered.length === 0 && (
                            <div className="col-span-full bg-white/70 backdrop-blur-md rounded-2xl p-6 border border-white/30 text-center text-slate-600">
                                Belum ada kategori {filterType.toLowerCase()}.
                                Klik{" "}
                ...
                <div className="space-y-5">
                    <div className="grid grid-cols-[56px,1fr] gap-3 items-center">
                        ...
                        <div className="relative">
                            ...
                            <Emoji
                                open={showEmoji}
                                onSelect={(unicode) => setIcon(unicode)}
                    ...
                    <div className="flex items-center gap-2 text-sm text-slate-500">
                        ...
                        <span>
                            Pakai emoji buat icon kategori biar cepat dikenali.
                        </span>
                    </div>
                </div>
            </Modal>
