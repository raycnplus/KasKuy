...
const RDashboard = () => {
  ...
  return p.map((w) => w[0]).join("").toUpperCase();
    ...
    const fetchMonthlyCompare = async (month, year) => {
        const { data } = await api.get("/reports/monthly-compare", {
            params: { month, year },
        });
        return data;
    };
        ...
        async (withSpinner = false) => {
            ...
            try {
                ...
                const mapped = Array.isArray(raw)
                    ? raw.map((tx) => {
                          const { title, desc } = splitTitleAndDesc(
                              tx.title || tx.description || ""
                          );
                          const d = new Date(tx.date);
                          const catIcon =
                              tx.category?.icon ||
                              (Array.isArray(tx.category?.icons) &&
                              tx.category.icons.length
                                  ? tx.category.icons[0]
                                  : null) ||
                              getEmojiForCategory(
                ...
                const filtered = mapped
                    .filter((t) => t.month === monthNum && t.year === yearNum)
                    .map(({ month, year, ...t }) => t);
    ...
    const MonthYearPicker = () => {
        ...
        const handleMonthChange = (idx) => {
            if (
                ...
                (currentYear === todayYear && idx > todayMonth)
            )
                return;
        ...
        return (
            <div
                ref={pickerRef}
                className="absolute top-full right-0 mt-2 bg-white/95 backdrop-blur-md rounded-2xl p-6 shadow-2xl border border-white/30 z-[1000] min-w-[320px]"
            >
                                    ...
                                    balanceChange !== 0 ? (
                                        <p
                                            className={`inline-block text-sm font-medium ${
                                                balanceChange > 0
                                                    ? "text-emerald-600 border-2 border-emerald-600/40"
                                                    : "text-rose-600 border-2 border-rose-600/40"
                                            } px-2 py-1 rounded-lg mt-4`}
                                        >
                                            {balanceChange > 0 ? "+" : ""}
                                            {balanceChange}% dari bulan
                                            sebelumnya
                                        </p>
                                    ) : null}
                                </div>
                ...
                <div className="mb-8 relative z-0">
                    ...
                    <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
                        {[
                            ...
                            >
                                <div
                                    className={`w-12 h-12 rounded-xl bg-gradient-to-br ${a.color} flex items-center justify-center mb-3 shadow-lg group-hover:scale-110 transition-transform duration-300`}
                                >
                                        ...
                                        <div className="flex items-start justify-between">
                                            <div className="flex items-start gap-4 flex-1">
                                                <div className="relative z-[100]">
                                                    ...
                                                    <div
                                                        className={`absolute -bottom-1 -right-1 w-6 h-6 rounded-lg bg-gradient-to-br ${meta.color} flex items-center justify-center shadow-md`}
                                                    >
                                                ...
                                                <div className="flex-1 min-w-0">
                                                    ...
                                                    <div className="flex items-center gap-3 mb-3">
                                                        <span
                                                            className={`px-3 py-1 rounded-lg text-xs font-semibold ${meta.bg} text-slate-700`}
                                                        >
                                                            {tx.kategori}
                                                        </span>
