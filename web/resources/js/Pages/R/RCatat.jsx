      ...
      <div className="grid grid-cols-2 gap-3 sm:gap-4">
          ...
          <div className="flex items-center justify-center gap-2 sm:gap-3 h-full px-3 sm:px-4">
            <div
              className={`w-5 h-5 sm:w-6 sm:h-6 rounded-md flex items-center justify-center transition-colors duration-300 ${
                selectedType === "Pengeluaran" ? "bg-red-500" : "bg-slate-300"
              }`}
            >
              <ArrowUpRight
                className={`w-3 h-3 sm:w-4 sm:h-4 transition-colors duration-300 ${
                  selectedType === "Pengeluaran" ? "text-white" : "text-slate-500"
                }`}
              />
            </div>
            <h3
              className={`text-base sm:text-lg font-semibold transition-colors duration-300 ${
                selectedType === "Pengeluaran" ? "text-red-600" : "text-slate-600"
              }`}
            >
              Pengeluaran
            </h3>
          </div>
          {selectedType === "Pengeluaran" && <div className="absolute inset-0 rounded-xl sm:rounded-2xl bg-red-500/5 pointer-events-none"></div>}
        </button>
        <button
        ...
        >
          <div className="flex items-center justify-center gap-2 sm:gap-3 h-full px-3 sm:px-4">
            <div
              className={`w-5 h-5 sm:w-6 sm:h-6 rounded-md flex items-center justify-center transition-colors duration-300 ${
                selectedType === "Pemasukan" ? "bg-emerald-500" : "bg-slate-300"
              }`}
            >
              <ArrowDownRight
                className={`w-3 h-3 sm:w-4 sm:h-4 transition-colors duration-300 ${
                  selectedType === "Pemasukan" ? "text-white" : "text-slate-500"
                }`}
              />
            </div>
            <h3
              className={`text-base sm:text-lg font-semibold transition-colors duration-300 ${
                selectedType === "Pemasukan" ? "text-emerald-600" : "text-slate-600"
              }`}
            >
              Pemasukan
            </h3>
          </div>
          {selectedType === "Pemasukan" && <div className="absolute inset-0 rounded-xl sm:rounded-2xl bg-emerald-500/5 pointer-events-none"></div>}
        </button>
      </div>
    </div>
  );
...
const DraftItem = ({ draft, onEdit, onDelete }) => {
  ...
  const meta = (t) =>
    t === "Pemasukan"
      ? { grad: "from-emerald-500 to-emerald-600", Icon: ArrowDownRight, txt: "text-emerald-600" }
      : { grad: "from-red-500 to-rose-600", Icon: ArrowUpRight, txt: "text-red-600" };
      ...
      <div className="flex items-start justify-between">
        <div className="flex items-start gap-4 flex-1">
          <div className="relative">
            ...
            <div className={`absolute -bottom-1 -right-1 w-6 h-6 rounded-lg bg-gradient-to-br ${m.grad} flex items-center justify-center shadow-md`}>
              <m.Icon className="w-3 h-3 text-white" />
            </div>
          </div>
          <div className="flex-1 min-w-0">
            <div className="mb-1">
              <h3 className="font-bold text-slate-800 text-lg leading-tight truncate">{draft.title || "Tanpa judul"}</h3>
              {draft.desc && <p className="text-slate-500 text-sm line-clamp-2">{draft.desc}</p>}
            </div>
            <div className="flex items-center gap-3">
              ...
              {draft.date && (
                <div className="flex items-center gap-1 text-slate-500 text-sm">
                  ...
                  <span>{new Date(draft.date).toLocaleDateString("id-ID")}</span>
                </div>
              )}
            </div>
          </div>
        ...
        <div className="text-right ml-4 flex flex-col items-end gap-2">
          ...
          <div className="flex gap-2">
            <button onClick={() => onEdit(draft)} className="p-2 text-slate-500 hover:text-emerald-600 hover:bg-emerald-50 rounded-lg transition-colors">
              <Edit3 className="w-4 h-4" />
            </button>
            <button onClick={() => onDelete(draft.id)} className="p-2 text-slate-500 hover:text-red-600 hover:bg-red-50 rounded-lg transition-colors">
...
const RCatat = () => {
  ...
  const loadCategories = async () => {
    try {
      ...
      setCategories([
        ...
        { id: 2, name: "Bayar Sewa Kos", icon: "🏠", type: "Pengeluaran" },
        { id: 3, name: "Kebutuhan Rumah Tangga", icon: "🛒", type: "Pengeluaran" },
  ...
  const visibleCategories = useMemo(() => categories.filter((c) => c.type === selectedType), [categories, selectedType]);

  useEffect(() => {
    if (visibleCategories.length === 0) return;
    if (!visibleCategories.some((c) => Number(c.id) === Number(categoryId))) {
  ...
  const validateForm = () => title.trim().length > 0 && amount && parseFloat(String(amount).replace(/[^\d]/g, "")) > 0 && categoryId;
  ...
  const handleSubmit = async () => {
    ...
    try {
      const allItems = [];
      if (validateForm()) {
        const finalDescription = [title.trim(), desc].filter(Boolean).join(" — ") || null;
        allItems.push({
          type: selectedType,
          amount: parseFloat(String(amount).replace(/[^\d]/g, "")),
      ...
      allItems.push(
        ...drafts.map((draft) => ({
          type: draft.type,
          amount: draft.amount,
          description: [draft.title, draft.desc].filter(Boolean).join(" — ") || null,
  ...
  const handleDeleteDraft = (draftId) => setDrafts((prev) => prev.filter((d) => d.id !== draftId));

  const formatCurrency = (value) => new Intl.NumberFormat("id-ID").format(String(value).replace(/[^\d]/g, ""));
    ...
    return (
      <div className="w-full min-h-screen bg-gradient-to-br from-emerald-50 to-teal-100 flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-emerald-700 font-medium">Memuat Form...</p>
        </div>
      </div>
    );
  }
      ...
      <div className="p-4 sm:p-6 lg:p-8 relative z-10 max-w-7xl mx-auto pb-32">
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6 sm:mb-8 gap-4">
          <div className="flex items-center gap-x-2">
            ...
            <h1 className="text-xl sm:text-2xl font-semibold text-slate-800">Catat Transaksi</h1>
          </div>
        ...
        {authError && (
          <div className="mb-4 px-4 py-3 rounded-xl bg-amber-50 text-amber-700 border border-amber-200">
            Tidak dapat memuat kategori. Silakan login terlebih dahulu.
          </div>
        )}
        ...
        <div className="bg-white/60 backdrop-blur-xl rounded-2xl sm:rounded-3xl p-4 sm:p-6 lg:p-8 shadow-lg border border-white/20 mb-20">
          <TransactionToggle selectedType={selectedType} onTypeChange={setSelectedType} />
          ...
          <div className="mt-4 sm:mt-6">
            <div className="grid gap-3 sm:gap-4">
              ...
              <div className="relative">
                <input
                  type="text"
                  value={amount ? `Rp ${formatCurrency(amount)}` : ""}
              ...
              <div className="w-full bg-gray-500/5 border-2 border-gray-200/20 focus:border-emerald-200 focus:outline-none rounded-xl rounded-b-3xl p-4 sm:p-5">
                <p className="font-medium text-slate-800 mb-3 sm:mb-5 text-base sm:text-xl">Kategori</p>
        ...
        {drafts.length > 0 && (
          <div className="bg-white/50 backdrop-blur-xl rounded-2xl sm:rounded-3xl p-4 sm:p-6 shadow-lg border border-white/20 mb-20">
            <div className="flex items-center justify-between mb-4">
              ...
              <button onClick={() => setDrafts([])} className="text-red-500 hover:text-red-600 text-sm font-medium flex items-center gap-1">
                <X className="w-4 h-4" />
                Hapus Semua
              </button>
            </div>
            <div className="space-y-3">
              {drafts.map((draft) => (
                <DraftItem key={draft.id} draft={draft} onEdit={handleEditDraft} onDelete={handleDeleteDraft} />
              ))}
            </div>
          </div>
        )}
        ...
        {showSuccess && (
          <div className="fixed top-4 left-4 right-4 sm:top-6 sm:left-6 sm:right-6 z-50 flex justify-center">
            <div className="bg-emerald-500 text-white px-6 py-3 rounded-xl shadow-lg flex items-center gap-2 animate-bounce">
              ...
              <span className="font-medium">Transaksi berhasil dicatat!</span>
            </div>
          </div>
        )}
      </div>
      ...
      <div className="fixed bottom-0 left-0 w-full bg-white/70 backdrop-blur-md px-4 sm:px-6 py-3 sm:py-4 flex gap-3 z-50 border-t border-white/20">
        <button
          ...
          className={`flex-1 py-3 rounded-xl font-semibold text-base sm:text-lg shadow-sm transition ${
            validateForm() ? "bg-emerald-200 text-emerald-700 hover:bg-emerald-300" : "bg-gray-200 text-gray-500 cursor-not-allowed"
          }`}
        >
          Tambah Item
        </button>
        <button
          ...
          disabled={(!validateForm() && drafts.length === 0) || isSubmitting}
          className={`flex-1 py-3 rounded-xl text-white font-semibold text-base sm:text-lg shadow-lg transition ${
            (validateForm() || drafts.length > 0) && !isSubmitting ? "bg-gradient-to-br from-emerald-500 to-emerald-600 hover:from-emerald-600 hover:to-emerald-700" : "bg-gray-400 cursor-not-allowed"
          }`}
        >
          {isSubmitting ? "Menyimpan..." : "Catat Item"}
          {drafts.length > 0 && !isSubmitting ? ` (${drafts.length + (validateForm() ? 1 : 0)})` : ""}
        </button>
      </div>
    </div>
  );
