...
export default function Settings() {
  ...
  const canSubmitPw = useMemo(() => {
    return pw.current && pw.next.length >= 6 && pw.next === pw.confirm && !loadingPw;
  }, [pw, loadingPw]);
      ...
      <div className="p-4 sm:p-6 lg:p-8 relative z-10 max-w-7xl mx-auto pb-28">
        <div className="flex items-center justify-between mb-6 sm:mb-8">
          <div className="flex items-center gap-2">
            ...
            <h1 className="text-xl sm:text-2xl font-semibold">Pengaturan</h1>
          </div>
        </div>

        <div className="grid lg:grid-cols-[1.15fr,2fr] gap-6">
          <div className="space-y-6">
            <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="bg-white/90 backdrop-blur-xl rounded-3xl p-6 border border-white/40 shadow-2xl">
              <div className="flex items-start gap-4">
                <div className="w-14 h-14 rounded-2xl bg-emerald-100 text-emerald-700 grid place-items-center">
                  <UserCircle2 className="w-8 h-8" />
                </div>
                <div className="min-w-0">
                  <div className="text-slate-900 font-semibold text-lg truncate">{profile?.name || "Pengguna"}</div>
                  <div className="text-slate-600 text-sm truncate">@{profile?.username || "username"}</div>
              ...
              <div className="mt-5 grid grid-cols-2 gap-3 text-sm">
                <div className="rounded-2xl ring-1 ring-emerald-200/60 bg-emerald-50/70 p-3">
                  <div className="text-emerald-700 font-semibold">Tema</div>
                  <div className="text-emerald-600/80">{theme === "system" ? "Mengikuti Sistem" : theme === "dark" ? "Gelap" : "Terang"}</div>
                </div>
                <div className="rounded-2xl ring-1 ring-cyan-200/60 bg-cyan-50/70 p-3">
                  <div className="text-cyan-700 font-semibold">Bahasa</div>
                  <div className="text-cyan-600/80">Indonesia</div>
                </div>
              </div>
            </motion.div>
              ...
              <div className="grid gap-3">
                ...
                >
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <div className="font-semibold text-slate-800">Kelola Teman</div>
                      <div className="text-xs text-slate-600">Tambah/kelola teman untuk split bill</div>
                    </div>
                  </div>
                  <span className="text-emerald-600 font-medium opacity-0 group-hover:opacity-100 transition">Buka →</span>
                </Link>
                ...
                >
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <div className="font-semibold text-slate-800">Kelola Kategori</div>
                      <div className="text-xs text-slate-600">Buat, edit, dan atur prioritas</div>
                    </div>
                  </div>
                  <span className="text-cyan-600 font-medium opacity-0 group-hover:opacity-100 transition">Buka →</span>
                </Link>
              </div>
            </motion.div>

            <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="bg-white/90 backdrop-blur-xl rounded-3xl p-6 border border-white/40 shadow-2xl">
              <h2 className="text-sm font-bold text-slate-900 mb-3">Aksi Akun</h2>
              <button
                onClick={logout}
          ...
          <div className="space-y-6">
            <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="bg-white/90 backdrop-blur-xl rounded-3xl p-6 border border-white/40 shadow-2xl">
              <h2 className="text-lg font-bold mb-5">Preferensi</h2>
              <div className="grid sm:grid-cols-3 gap-4">
                ...
                <div className="space-y-2">
                  <div className="text-sm font-medium text-slate-700 flex items-center gap-2">
                    <BellRing className="w-4 h-4" /> Notifikasi
                  </div>
                  <div className="flex items-center gap-3">
                    <Toggle checked={notif} onChange={setNotif} />
                    <span className="text-sm text-slate-600">{notif ? "Aktif" : "Nonaktif"}</span>
                  </div>
                </div>
              </div>
            </motion.div>
            ...
            <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="bg-white/90 backdrop-blur-xl rounded-3xl p-6 border border-white/40 shadow-2xl">
              ...
              <div className="grid sm:grid-cols-3 gap-4">
                <div className="space-y-2">
                  <div className="text-sm font-medium text-slate-700">Password Saat Ini</div>
                  <input
                ...
                <div className="space-y-2">
                  <div className="text-sm font-medium text-slate-700">Password Baru</div>
                  <input
                    type={showPw ? "text" : "password"}
                    value={pw.next}
                ...
                <div className="space-y-2">
                  <div className="text-sm font-medium text-slate-700">Konfirmasi</div>
                  <div className="relative">
                    ...
                    >
                      {showPw ? <EyeOff className="w-5 h-5 text-slate-500" /> : <Eye className="w-5 h-5 text-slate-500" />}
                    </button>
                  </div>
                </div>
              </div>
              <div className="mt-4 flex justify-end">
                <button
                  ...
                  className={`px-5 py-2 rounded-xl font-semibold shadow ${
                    canSubmitPw ? "bg-emerald-600 text-white" : "bg-slate-200 text-slate-500 cursor-not-allowed"
                  }`}
                >
        ...
        {toast && (
          ...
          >
            <div className="flex items-center gap-2">
              ...
              <button onClick={() => setToast(null)} className="p-1 rounded hover:bg-white/10">
                <X className="w-4 h-4" />
              </button>
            </div>
          </motion.div>
